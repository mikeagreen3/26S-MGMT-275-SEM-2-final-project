...
import { batteryTone } from '../lib/utils'

export default function PropertiesList({ properties, onView }) {
  return (
          ...
          <tbody>
            {properties.map((p) => (
              <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50/50">
                ...
                <td className={`px-5 py-3 font-medium ${batteryTone(p.battery) === 'bad' ? 'text-[#dc2626]' : batteryTone(p.battery) === 'warn' ? 'text-[#d97706]' : 'text-[#16a34a]'}`}>
                  {p.battery}%
                </td>

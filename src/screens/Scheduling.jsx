...
export default function Scheduling({ properties, accessCodes, onAddCode, prefillLockId, onClearPrefill }) {
  const [input, setInput] = useState('')
  const [selectedLockId, setSelectedLockId] = useState(prefillLockId || '')
  ...
  const submit = async () => {
    ...
    let systemPrompt = SCHEDULING_SYSTEM_PROMPT.replace('__DATE__', todayStr)
    if (selectedLockId) {
    ...
    try {
      const reply = await callClaude(systemPrompt, `Request: ${input.trim()}`)
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {conflict.suggested_adjustment && (
                  <div className="text-sm text-amber-900 mt-2"><span className="font-medium">Suggestion:</span> {conflict.suggested_adjustment}</div>
                )}
                <div className="flex gap-2 mt-3">
                  <Button variant="primary" size="sm" onClick={() => addCodeFromResponse({ window: conflict.suggested_adjustment || (parsed.start && parsed.end ? formatTimeRange(parsed.start, parsed.end) : parsed.schedule) })}>Yes, adjust</Button>
                  <Button variant="secondary" size="sm" onClick={() => addCodeFromResponse()}>Enter manually</Button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-[#fde68a] bg-[#fefce8] p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-yellow-900 mt-1">I'm only {Math.round(confidence * 100)}% confident I parsed this correctly. Please confirm the details above before I push the code.</div>
                <div className="flex gap-2 mt-3">
        ...
        {(confirmed || (!conflict?.detected && confidence >= 0.7)) && (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-emerald-900">{confirmed ? 'Access scheduled' : 'Ready to schedule'}</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <select value={selectedLockId} onChange={(e) => setSelectedLockId(e.target.value)} className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm text-slate-800 mb-4 bg-white">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          ...
          {examples.map((ex) => (
            <button key={ex} onClick={() => setInput(ex)} className="text-xs px-2.5 py-1 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 transition">{ex}</button>
          ))}
        </div>

...
| Aspect | v1 | v2 | Why |
|---|---|---|---|
| Hero framing | Pre-arrival check leads; NL scheduling is secondary | Workflow story leads; pre-arrival check is the payoff | RICE found combined access reliability framing wins on every axis except effort. See `rice-prioritization.md`. |
| Booking sync | Not mentioned | Mentioned as job 1 of 4 | Product brief lists it as the highest-leverage host workflow. |
| Audience scope | Host-focused | Host-focused (held) | Splitting the message weakens positioning; non-hosts are not harmed because features stay opt-in. |
| Market size | 2.5M hosts (no source) | "More than 2 million short-term rental hosts in the United States" | Tightened to a defensible US-only number consistent with Airbnb 10-K. |
| Exec quote | "a Senior Director of Product Management" | "Maria Chen, SVP of Connected Hardware, Allegion" | Realism. Fictional but plausible. |
| Pricing line | "$4.99/month Schlage Pro tier" | Removed | Not in strategy or brief; pricing belongs in Internal FAQ. |
| Voice | Em-dashes, "intelligence layer" (×2), "transforms... into a proactive property operations partner," "closed-loop workflow" | All buzzwords/em-dashes removed; "put that data to work" kept | Per Mike's voice rules (no em-dash, no buzzwords). |
| Length | ~370 words | ~360 words | Comfortable under 1.5-page cap. |
